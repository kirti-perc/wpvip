...
while ( have_posts() ) {
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<div class="entry-content">
			...
			wp_link_pages(
				array(
					'before'   => '<nav class="page-links" aria-label="' . esc_attr__( 'Page', 'pstheme' ) . '">',
					'after'    => '</nav>',
					/* translators: %: page number. */
					'pagelink' => esc_html__( 'Page %', 'pstheme' ),
				)
			);
			?>
		</div><!-- .entry-content -->
		...
		<footer class="entry-footer default-max-width">
			...
			if ( wp_get_post_parent_id( $post ) ) {
				...
				printf(
					...
					esc_html__( 'Published in %s', 'pstheme' ),
			...
			if ( $metadata ) {
				printf(
					...
					esc_html_x( 'Full size', 'Used before full size attachment link.', 'pstheme' ), // phpcs:ignore WordPress.Security.EscapeOutput
